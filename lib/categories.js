...
configuration.addListener((config) => {
  ...
  if (!config.increment && !categories.has('default')) {
    configuration.throwExceptionIf(
      config,
...
const setup = (config) => {
  ...
  categoryNames.forEach((name) => {
    ...
    const categoryAppenderNames = [];
    category.appenders.forEach((appender) => {
      categoryAppenderNames.push(appender)
      categoryAppenders.push(appenders.get(appender));
      debug(`Creating category ${name}`);
      categories.set(
        name,
        {
          appenderNames: categoryAppenderNames,
...
const hasCategory = (category) => categories.has(category)
const getCategory = (category) => categories.get(category)
const getCategoryValues = () => Array.from(categories.values)
